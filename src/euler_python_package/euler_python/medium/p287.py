...
def problem287():
    ...
    def compressed_length(xstart, xend, ystart, yend):
        if xstart * xstart + ystart * ystart > RADIUS_SQUARED:  # All white
            return 2
        elif (xend - 1) * (xend - 1) + (yend - 1) * (
                yend - 1
        ) <= RADIUS_SQUARED:  # All black
            return 2
            ...
            return (
                    1
                    + compressed_length(xstart, xmid, ymid, yend)
                    + compressed_length(xmid, xend, ymid, yend)  # Top left
                    + compressed_length(xstart, xmid, ystart, ymid)  # Top right
                    + compressed_length(xmid, xend, ystart, ymid)  # Bottom left
            )  # Bottom right

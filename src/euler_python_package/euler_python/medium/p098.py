    ...
    for (key, words) in anagrams.items():
        for i in range(len(words)):
            for j in range(i + 1, len(words)):
                assignments = {}
                ans = max(
                    max_square_pair(words[i], words[j], 0, assignments, [False] * 10),
                    ans,
                )
    return ans
...
def max_square_pair(a, b, index, assignments, isdigitused):
    if index == len(a):
        if (
                a[0] in assignments
                and assignments[a[0]] == 0
                or b[0] in assignments
                and assignments[b[0]] == 0
        ):
            return 0
        ...
        for i in range(10):
            if not isdigitused[i]:
                ...
                result = max(
                    max_square_pair(a, b, index + 1, assignments, isdigitused), result
                )

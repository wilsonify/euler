"""
problem 12
"""
import itertools

from euler_python.utils import eulerlib


# Returns the number of integers in the range [1, n] that divide n.
def num_divisors(input_n):
    """
    :param input_n:
    :return:
    """
    end = eulerlib.sqrt(input_n)
    result = sum(2
                 for i in range(1, end + 1)
                 if input_n % i == 0)
    if end ** 2 == input_n:
        result -= 1
    return result


def triangle_number_factors(divisors=500):
    """
       The sequence of triangle numbers is generated by adding the natural
       numbers. So the 7^th triangle number would be 1 + 2 + 3 + 4 + 5 + 6 + 7 =
       28. The first ten terms would be:

                        1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...

       Let us list the factors of the first seven triangle numbers:

          1: 1
          3: 1,3
          6: 1,2,3,6
         10: 1,2,5,10
         15: 1,3,5,15
         21: 1,3,7,21
         28: 1,2,4,7,14,28

       We can see that 28 is the first triangle number to have over five
       divisors.
    """
    triangle = 0
    for i in itertools.count(1):
        triangle += i  # This is the ith triangle number
        if num_divisors(triangle) > divisors:
            break
    return triangle


def problem012():
    """
    What is the value of the first triangle number to have over five hundred divisors?

    :return:
    """
    return triangle_number_factors(divisors=500)


if __name__ == "__main__":
    print(problem012())
